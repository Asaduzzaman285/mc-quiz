...
const ProductsPage = () => {
    ...
    const fetchProducts = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${API_BASE}/api/v1/public/portfolio/products`);
            if (res.data?.success) {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            uploadData.append('file_path', 'portfolio/products/');
    ...
    const handleSave = async (e) => {
        ...
        try {
            const url = isEditing
                ? `${API_BASE}/api/v1/admin/portfolio/products/${formData.id}`
                : `${API_BASE}/api/v1/admin/portfolio/products`;
            ...
            if (res.data?.success) {
                Swal.fire({ icon: 'success', title: 'Success', text: 'Product saved!', timer: 2000 });
                setShowModal(false);
                fetchProducts();
            }
        } catch (err) {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirm.isConfirmed) {
            try {
                const token = localStorage.getItem('authToken');
                await axios.delete(`${API_BASE}/api/v1/admin/portfolio/products/${id}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    const openAddModal = () => {
        setIsEditing(false);
        setFormData({ id: null, product_category_id: '', title: '', description: '', image_path: '' });
        setShowModal(true);
    };
    ...
    return (
        <div style={{ padding: '16px', backgroundColor: '#F5F5F5', overflowX: 'hidden', minHeight: '100vh' }}>
            <div className="mt-5 d-flex justify-content-between align-items-center mb-4">
                <h1 style={{ fontSize: '20px', fontWeight: 'bold', margin: 0 }}>Product Categories (Subproducts)</h1>
                        ...
                        <table className="table table-bordered table-hover align-middle" style={{ fontSize: '13px' }}>
                            <thead className="table-light">
                                <tr>
                                    ...
                                    <th>Category/Subproduct Name</th>
                                    <th>Parent Collective Product</th>
                                    ...
                                    products.map(prod => (
                                        <tr key={prod.id}>
                                            <td className="text-center">
                                                {prod.image_path ? (
                                                    <img src={`${API_BASE}${prod.image_path}`} alt={prod.title} style={{ width: '50px', height: '50px', objectFit: 'cover', borderRadius: '4px' }} />
                                            ...
                                            <td><span className="badge bg-secondary">{prod.category?.name || 'Unknown Category'}</span></td>
                                            <td>
                                                <div style={{ maxHeight: '40px', overflow: 'hidden', textOverflow: 'ellipsis' }}>{prod.description}</div>
                                            </td>
                                            <td className="text-center">
                                                <button onClick={() => openEditModal(prod)} className="btn btn-sm btn-light me-2"><Edit size={14} className="text-primary" /></button>
                                                <button onClick={() => handleDelete(prod.id)} className="btn btn-sm btn-light"><Trash2 size={14} className="text-danger" /></button>
                                            </td>
                                        </tr>
                                    ))
                                )}
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} backdrop="static" size="lg">
                <Modal.Header closeButton>
                    <Modal.Title>{isEditing ? 'Edit Category' : 'Register Category'}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form onSubmit={handleSave}>

                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold small">Assign to Collective Product *</Form.Label>
                            <Form.Select
                                required
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold small">Category / Subproduct Name *</Form.Label>
                            <Form.Control required type="text" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} placeholder="E.g. Caustic Soda (Lye & Flakes)" />
                        </Form.Group>

                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold small">Category Details / Description</Form.Label>
                            <Form.Control as="textarea" rows={4} value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} placeholder="Tech specifications or usage..." />
                        </Form.Group>

                        <Form.Group className="mb-3">
                            <Form.Label className="fw-bold small">Category Image</Form.Label>
